...
public class Tablero {
        ...
        for (int i = ini; i < cant; i++) {
            for (int j = 0; j < x; j++) {
                if (tablero[j][i].getEsColor() == !juegoEnBlancas) {
                    fichas[tipoFicha][(cont - 1)] = new Ficha(!esBlanca, componerNumeroString(cont),
                            new Coordenada(j, i), debeAscender, this);
    ...
    public boolean elimiarFicha(Ficha ficha, Coordenada posicionFinal){
        boolean respuesta = false;
        Celda tmpCelda = null;
        ...
        if (ficha != null) {
            Celda iniCelda = this.getCelda(ficha.getCoordenada());
            if (!(tmpCelda = tablero[posicionFinal.getX()][posicionFinal.getY()]).ocupadaPorFicha()) {
                tmpCelda.setFicha(null, posicionFinal.getX(), posicionFinal.getY());
                iniCelda.setFicha();
                respuesta = true;
            }
        }
